class MovingAverage:
    ...
    def update_average(self, new_value):
        if len(self.data) == self.interval_len:
            self.average = (self.average * len(self.data) + new_value - self.data[0]) / float(self.interval_len)
            self.data.pop(0)
        else:
            self.average = (self.average * len(self.data) + new_value) / float((len(self.data) + 1))
